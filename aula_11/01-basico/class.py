...
class ProcessadorCSV: #INICIALIZADOR
    def __init__(self, arquivo_csv):
        self.arquivo_csv = arquivo_csv
        self.df = None
    
    def carregar_csv(self): #METODO
        # Carregar o arquivo CSV em um DataFrame
        self.df = pd.read_csv(self.arquivo_csv)
    
    def remover_celulas_vazias(self): #METODO
        # Verificar e remover células vazias
        self.df = self.df.dropna()
    
    def filtrar_por_estado(self, estado): #METODO
        # Filtrar as linhas pela coluna estado
        self.df = self.df[self.df['estado'] == estado]
    
    def processar(self, estado): #METODO
